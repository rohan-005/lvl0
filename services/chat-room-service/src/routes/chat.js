...
// Configure Multer Storage for Uploads inside chat-room-service/uploads/
...
router.get("/:roomId/:channel", protect, async (req, res) => {
  try {
    ...
    console.error("[chat-room-service] Failed to fetch chat history:", err);
...
router.get("/users", protect, async (req, res) => {
  try {
    ...
    console.error("[chat-room-service] Failed to fetch user directory:", err);
...
router.get("/rooms", protect, async (req, res) => {
  try {
    ...
    console.error("[chat-room-service] Error fetching rooms", err);
...
router.post("/rooms", protect, async (req, res) => {
  try {
    ...
    console.error("[chat-room-service] Error creating room", err);
    res.status(500).json({ message: "Server Error" });
  }
});
