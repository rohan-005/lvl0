...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    console.log(`[chat-room-service socket] Connected: ${socket.user.name} (${socket.id})`);
    ...
    socket.on("message:send", async ({ roomId, channel, messageText, attachmentUrl }) => {
      ...
      try {
        ...
        console.error("[chat-room-service socket] Failed to send message", err);
    ...
    socket.on("message:vote", async ({ messageId, type }) => {
      try {
        ...
        console.error("[chat-room-service socket] Voting error", error);
      }
    });
    ...
    socket.on("message:react", async ({ messageId, emoji }) => {
      try {
        ...
        console.error("[chat-room-service socket] Reaction error:", error);
      }
    });
    ...
    socket.on("message:moderate", async ({ messageId, action, reason }) => {
      ...
      try {
        ...
        console.error("[chat-room-service socket] Moderation error:", error);
      }
    });

    socket.on("disconnect", () => {
      console.log(`[chat-room-service socket] Disconnected: ${socket.user.name}`);
    });
  });
};
